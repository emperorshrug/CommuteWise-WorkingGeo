...
import { supabase } from "@/lib/supabase"; // Ensure you configured this!
...
import { toast } from "sonner"; // Better alerts
...
export default function Auth() {
  ...
  // Sync with store
  useEffect(() => {
    if (hasAcceptedTerms) setTermsChecked(true);
  }, [hasAcceptedTerms]);

  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!termsChecked) return toast.error("Please accept the terms.");
  ...
  const handleGuest = () => {
    if (!termsChecked) return toast.error("Please accept the terms.");
    setAcceptedTerms(true); // Persist this!
    setGuestUser();
    navigate("/");
  };
        ...
        <CardContent className="space-y-6 pt-6">
          ...
          <div className="flex items-start space-x-3 bg-slate-50 p-4 rounded-xl border border-slate-100">
            ...
            >
              ...
              and consent to location tracking to find routes within Quezon
              City.
            </label>
          </div>
